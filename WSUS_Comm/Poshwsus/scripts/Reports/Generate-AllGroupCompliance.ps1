...
Foreach ($i in $TargetGroups) { 
    ...
    Import-Module .\Generate-ServersComplianceReportGroupv2.ps1 -Verbose
    Generate-ServersComplianceReportGroupv2 -WsusServer $WsusServer -TargetGroup $i -CSVPath $CSVPath\$i -ReportName $i"-$(Get-Date -Format MMddyyy)"
     Write-Host -ForegroundColor Magenta "Generating a Detailed report of updates required on non-compliant machines."
    Write-Host -ForegroundColor Magenta  "This may take some time..."
    
   Get-PSWSUSClientsInGroup -Name $i |Get-PSWSUSUpdatePerClient -UpdateScope (New-PSWSUSUpdateScope -ApprovedStates Any)  | Where-Object -Property Updateapprovalaction -Like install | Where -Property UpdateInstallationstate -NotLike "NotApplicable"| Where-Object {$_.UpdateInstallationState -notlike "Installed"}| select computername, updatekb, updateinstallationstate,updateapprovalaction | where {$_.UpdateInstallationState -notlike "Installed"} | Export-Csv $CSVPath"\"$i"\$i-$(Get-Date -Format MMddyyy)-Detailed.csv" -NoTypeInformation
